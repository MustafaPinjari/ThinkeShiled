...
def score_agency_tender(self, submission_id: int) -> None:
    ...
    2. Call the existing score_tender pipeline on the Tender.
    ...
    - Creates internal Alert for all ADMIN users.
    ...
    try:
        # Call the existing score_tender pipeline
        # The pipeline writes FraudRiskScore, RedFlag records, etc.
        from scoring.tasks import score_tender  # noqa: PLC0415
        score_tender(tender.pk)

        # Reload the fraud risk score from DB
        score_value = None
        try:
            score_value = float(tender.fraudriskscore.score)
        except Exception:
            logger.warning(
                "score_agency_tender: No FraudRiskScore found for tender pk=%d after scoring.",
                tender.pk,
            )

        # Update TenderSubmission status based on score
        if score_value is not None:
            if score_value >= 70:
                try:
                    submission.transition_to(
                        SubmissionStatus.UNDER_REVIEW, actor=None
                    )
                except ValueError:
                    pass  # Already transitioned or invalid — log and continue
            elif score_value < 40:
                try:
                    submission.transition_to(
                        SubmissionStatus.CLEARED, actor=None
                    )
                except ValueError:
                    pass

        # Write SCORE_COMPUTED AuditLog entry
        AuditLog.objects.create(
        ...
        # If score >= 70, create alerts for agency admin/officer users
...
def _handle_scoring_permanent_failure(submission_id: int, error: str) -> None:
    ...
    - Creates internal AuditLog alerts for all ADMIN users.
    Requirements: 10.7
    """
    from audit.models import AuditLog, EventType  # noqa: PLC0415
    from authentication.models import User, UserRole  # noqa: PLC0415
    ...
    for admin in admin_users:
        try:
            ...
            logger.error(
                "Failed to write AuditLog for scoring failure (submission_id=%d): %s",
                submission_id,
                log_exc,
            )
    ...
    logger.error(
        ...
        "Created %d admin alert(s). Error: %s",
        submission_id,
        len(admin_users),
        error,
    )
